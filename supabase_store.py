...
def get_active_signal(symbol, exchange):
    ...
    r = requests.get(url("signals"), headers=headers(), params=p, timeout=20)
    r.raise_for_status()
    rows = r.json()
...
def create_signal(row, expires_hours=12):
    ...
    payload = {
        ...
        "tp1": float(row["tp1"]),
        "tp2": float(row["tp2"]),
        "rsi_15m": float(row["rsi_15m"]),
        "relative_volume": float(row["rel_volume_15m"]),
        "spread_pct": float(row["spread_pct"]),
        "cross_confirmed": bool(row.get("cross_exchange", False)),
        "status": "ACTIVE",
        "tp1_hit": False,
        "tp2_hit": False,
        "stop_hit": False,
        "max_price": float(row["price"]),
        "min_price": float(row["price"]),
        "max_return_pct": 0.0,
...
def list_signal_events(limit=1000):
    p = {
        "select": "*",
        "order": "created_at.desc",
        "limit": str(limit),
    }
    r = requests.get(url("signal_events"), headers=headers(), params=p, timeout=20)
    r.raise_for_status()
    return r.json()
